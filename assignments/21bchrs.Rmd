...
3. The polynomial $g(x) = x^4 + x^3 + 1$ generates a BCH code in $\mathbb{Z}_2[x]/(x^{15}+1)$. Use the method on pp. 438-439 (or the special case given in the slides) to correct the single error in the received word $r(x) = x^{14}+x^{13}+x^{10}+x^9+x^7+x^5+x+1$. Give the error vector, along with the corrected code word.
...
5. Consider the code of Problem #4. Use the method on pp. 438-439 (or the special case given in the slides) to correct the error in the following received word. 
