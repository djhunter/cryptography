...
2. In the last programming assignment, the hash function `miniSHA` defaults to a 32-bit hash.
    a. The examples say that `birthdayAttack(miniSHA, giveUp = 100000)` "probably" should find a collision. Use the Birthday Theorem to calculate the probability of finding a collision in this case.
    b. The examples say that `birthdayAttack(miniSHA, giveUp = 1000000)` "almost certainly" should find a collision. Use the Birthday Theorem to calculate the probability of finding a collision in this case.
