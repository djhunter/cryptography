...
```
-poll "How many keys would you have to try in a brute-force ciphertext-only attack on a Vigenère cipher of key length n?" n n^2 26n 26^n n^26 n! 26!
```
...
## Summary: finding the key length {.build}
...
```
-poll "Guess the first element of the key, based on this output." a:0 d:3 h:7 l:11 n:13 q:16
```
...
- Let $\mathbf{u}_0$ be the letter frequencies of the letters in position $1, k+1, 2k+1, 3k+1, \ldots$ of the ciphertext, where $k$ is the key length.
- If $\mathbf{u}_1, \mathbf{u}_2, \ldots, \mathbf{u}_{25}$ are all the possible shifts of $\mathbf{u}_0$, and $\mathbf{v}$ is the vector of English letter frequencies, then the $\mathbf{u}_i$ that maximizes $\mathbf{u}_i \cdot \mathbf{v}$ is the most likely shift.
- That is, the first element of the key is $i$.
- Repeat. For the $p$th element of the key, look at the letters in position $p, k+p, 2k+p, 3k+p, \ldots$.
...
u <- letterFreq(skipString(ciphertext,1,9))
for(i in 0:25) {
  cat(paste("shift =", i, "\t", " u_i dot v =", u %*% shiftVec(englishFreqs, i), "\n"))
}
```
...
englishFreqs <- c(0.082,0.015,0.028,0.043,0.127,0.022,0.020,0.061,0.070, 0.002,0.008,0.040,0.024,
                  0.067,0.075,0.019,0.001,0.060,0.063,0.091,0.028,0.010,0.023,0.001,0.020,0.001)
u <- letterFreq(skipString(ciphertext,1,9))
matchFreqs <- sapply(0:25, function(i){u %*% shiftVec(englishFreqs, i)})
matchFreqs
...
for(p in 1:9) {
  u <- letterFreq(skipString(ciphertext,p,9))
  matchFreqs <- sapply(0:25, function(i){u %*% shiftVec(englishFreqs, i)})
