...
## Recall: RSA {transition="slide-in fade-out"}
...
## Computational cost of RSA? {transition="fade-in slide-out"}
...
- How do passwordless logins work in ssh?
...
- **The Prime Number Theorem.** (Hadamard, 1896) [Short proof](https://people.mpim-bonn.mpg.de/zagier/files/doi/10.2307/2975232/fulltext.pdf)
...
## Testing primality by brute force

Keep a big list of primes. To determine if $n$ is prime, test all the prime numbers up to $\sqrt{n}$ to see if any divide $n$.

> Exercise: If $n$ has 60 digits, how big will this list need to be? 

## Recall Euler's Theorem

> **Euler's Theorem:** If $\gcd(a,n)=1$, then $a^{\phi(n)} \equiv 1 \pmod n$.

An immediate corollary is known as **Fermat's Little Theorem**.

> If $p$ is prime, then $a^{p-1} \equiv 1 \pmod{p}$, for any $a < p$. 

## Fermat's Little Theorem {.smaller}
